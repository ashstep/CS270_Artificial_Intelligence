'''
    Using NCAA College Basketball statistics for the 2016- 2017 season, predict whether the “team” defeats the opponent.
...
namevarsforEC = ["Date","Team","Opponent","Opponent Location","Neutral Site","Team Result", "Team Location", "Game Type"]
...
#EC added
def betterStatThanOpponentExtraCredit(gameLoad,item):
    """Returns True if the given stat is better than the opponent's stat"""
    return float(gameLoad['Team '+item]) > float(gameLoad['Opponent '+item])
#EC added
def transformToBooleanFeaturesforExtraCredit(gameLoad):
    """Transforms the game features to boolean features regarding whether you are doing better or worse than the opponent"""
    res = dict()
    record_vars = ['Team Margin','Team Differential','Opponent Differential']
    for (f,v) in gameLoad.iteritems():
        if f not in statvars_team_avgs and f not in statvars_opp_avgs and f not in namevarsforEC and f not in record_vars:
            print "inside f is: ", f
            print "inside v is: ", v       
            res[f] = int(v)
    res["Differential_better"] = 1 if gameLoad["Team Differential"] > gameLoad['Opponent Differential'] else 0
    for f in statvars:
        print "statvars: ", statvars
        print "gameLoad is: ", gameLoad
        res[f+"_avg_better"] = 1 if betterStatThanOpponentExtraCredit(gameLoad,f) else 0
        res[f+"Allowed_avg_better"] = 1 if betterStatThanOpponentExtraCredit(gameLoad,f) else 0
    return res
...
def classifier_accuracy(probabilistic_classifier,p_threshold,testset,target):
    ...
    if u get 0 prob is not actualy zero
    """
    n = len(testset)
    num_tp = 0
    num_fp = 0
    ...
    for item in testset:
        ...
        #getting values
        if actually_pos == True and predict_pos == True:
            num_tp = num_tp + 1
        if actually_pos == True and predict_pos == False:
            num_fn = num_fn + 1        
        if actually_pos == False and predict_pos == True:
            num_fp = num_fp + 1
        if actually_pos == False and predict_pos == False:
            num_tn = num_tn + 1

    if (float(num_tp + num_fp) != 0):
        precision = (num_tp)/ float(num_tp + num_fp)
    else:
        precision = 0

    if (float(num_tp + num_fn) != 0):
        recall = (num_tp)/ float(num_tp + num_fn)
    else:
        recall = 0

    if (float(num_tp + num_tn + num_fp + num_fn) != 0):
        accuracy = (num_tp + num_tn)/ float(num_tp + num_tn + num_fp + num_fn)
    else:
        accuracy = 1
...
if __name__=="__main__":
    ...
    #prediction_variables = ['Score_avg_better', 'ScoreAllowed_avg_better', 'losses_better','wins_better' ]
    #prediction_variables = ['ScoreAllowed_avg_better']
    #prediction_variables = ['losses_better']
